import { GetAIExportUrl } from "@/lib/utils/geturl";
...
export const aiExportService = {
    ...
    async reconstructProject(fileTree: FileTreeNode[]): Promise<FileTreeNode[]> {
        try {
            ...
            console.log("RECONSTRUCTED DATA:", JSON.stringify(response.reconstructed, null, 2));
    ...
    async checkHealth(): Promise<HealthCheckResponse> {
        try {
            const url = GetAIExportUrl(AI_EXPORT_ENDPOINTS.HEALTH);
